...
def custom_score_2(game, player):
    ...
    Heuristic
    ---------
    Look for a move ahead to calculate the distance
    """
    ...
    for move in own_moves:
        next_ply_board = game.forecast_move(move)
        own_score += len(next_ply_board.get_legal_moves(player))

    for move in opp_moves:
        next_ply_board = game.forecast_move(move)
        opp_score += len(next_ply_board.get_legal_moves(next_ply_board.get_opponent(player)))
...
def custom_score_3(game, player):
    ...
    Heuristic
    ---------
    Assing to moves in the center of the board highest value, adding 50% to the score, and the worst
    score to the moves in the corners, and along the edges.
    """
    ...
    own_moves = game.get_legal_moves(player)
    for move in own_moves:
        #Center of the board, best score: +50%
        if move in [(2, 2), (2, 3), (2, 4), (3, 2), (3, 3), (3, 4), (4, 2), (4, 3), (4, 4)]:
            score += score * 0.5
        #Corners of the board, worse position: +0%
        elif move in [(0, 0), (0, w - 1), (h - 1, 0), (h - 1, w - 1)]:
            score += score * 0.0
        #Along the edges, bad position: + 10%
        elif ((move[0] == 0 or move[0] == h - 1) and move[1] >= 1 and move[1] <= w - 2) or (
                (move[1] == 0 or move[1] == w - 1) and move[0] >= 1 and move[0] <= h - 2):
            score += score * 0.10
        #Any other position: + 20%
        else:
            score += score * 0.20
        ...
        **********************************************************************
        ...
        def maxvalue(game, depth):
            """Implement max search algorithm for minimax as described in AIMA pseudo-code.
             ...
             Returns
             -------
             v: maximum value, or +/- inf if no moves available
             """
            ...
            # Check using score function
            if depth == 0:
                return self.score(game, self)

            # v at -inf, as specified in pseudocode
            v = float("-inf")
            ...
            for move in possible_moves:
                v = max(v, minvalue(game.forecast_move(move), depth - 1))
        ...
        def minvalue(game, depth):
            """Implement min search algorithm for minimax as described in AIMA pseudo-code.
             ...
             Returns
             -------
             v: minimum value, or +/- inf if no moves available
             """
            ...
            for move in possible_moves:
                v = min(v, maxvalue(game.forecast_move(move), depth - 1))
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        def alpha_maxvalue(game, depth, alpha=float("-inf"), beta=float("inf")):
            ...
            for move in possible_moves:
                v = max(v, alpha_minvalue(game.forecast_move(move), depth - 1, alpha, beta))
                if v >= beta:
                    return v
                alpha = max(alpha, v)
        ...
        def alpha_minvalue(game, depth, alpha=float("-inf"), beta=float("inf")):
            ...
            for move in possible_moves:
                v = min(v, alpha_maxvalue(game.forecast_move(move), depth - 1, alpha, beta))
                if v <= alpha:
                    return v
                beta = min(beta, v)
        ...
        # initialize best_move using the worst case
        best_move = (-1, -1)
        #best_move = possible_moves[0]
